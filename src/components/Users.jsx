...
const Users = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingUser) {
        if (isAdmin) {
          await updateUserRole(editingUser.id, formData.role, formData.store_id === NO_STORE_VALUE ? null : formData.store_id);
          toast({ title: "Role do usuário atualizada!", description: `${formData.name} agora é ${formData.role}.` });
        } else {
          toast({ title: "Apenas admin pode alterar a role!", variant: "destructive" });
        }
      } else {
        await addUser(userData);
        toast({ title: "Usuário criado!", description: `${formData.name} foi criado.` });
      }
      fetchInitialData();
      setShowDialog(false);
      resetForm();
  ...
  const handleToggleActive = async (userToToggle) => {
    setLoading(true);
    try {
      await updateUserRole(userToToggle.id, userToToggle.role, userToToggle.store_id); // updateUserRole para toggle active
      toast({
  ...
  const handleDelete = async (userId) => {
    ...
    setLoading(true);
    try {
      // deleteUser(userId); // Removido conforme instrução
      toast({ title: "Usuário removido!", description: "O usuário foi removido." });
      fetchInitialData();
    } catch (error)      {
  ...
  const isAdmin = users.find(u => u.id === getCurrentUser()?.id)?.role === USER_ROLES.ADMIN;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Dialog open={showDialog} onOpenChange={(isOpen) => {setShowDialog(isOpen); if(!isOpen) resetForm();}}>
          <DialogTrigger asChild><Button onClick={() => resetForm()}><Plus className="w-4 h-4 mr-2" />Novo Usuário</Button></DialogTrigger>
            ...
            <Card className={`glass-effect ${!user.active ? 'opacity-60' : ''}`}>
              ...
              <CardContent className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Loja: {user.stores?.name || (user.role === USER_ROLES.ADMIN ? 'Todas (Admin)' : 'Não definida')} {user.stores?.is_online_store ? "(Online)" : ""}</p>
